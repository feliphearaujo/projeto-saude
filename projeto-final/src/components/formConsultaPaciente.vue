<template>
          ...
          <v-card class="elevation-12" style="background-color: #fafafa;">
            ...
            <v-card-text>
              ...
              <v-form>
                <v-row>
                  <v-col cols="12" md="6">
                    <v-text-field disabled = "true" label="Paciente"  v-model="nome"  ></v-text-field>
                  </v-col>
                  <v-col cols="12" md="6">
                    <v-select :items="this.$store.state.medicos" label="Médico" item-title="nome" v-model="consulta.idmedico" item-value="idmedico"></v-select>
                  </v-col>
                  <v-col cols="12" md="6">
  ...
  export default {
    ...
    methods: {
        ...
        selecionaPaciente(){
            this.consulta.idpaciente = this.$store.state.pacienteLogin.idpaciente;
            this.nome = this.$store.state.pacienteLogin.nome;
      ...
      cadastrar() {
        ...
        getConsulta(){
                api.get("/consulta/paciente/"+this.consulta.idpaciente)
                    .then((res)=>{
